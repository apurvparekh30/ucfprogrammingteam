...
// Michael Goldwasser
//
// This version does additional verification of input validity and uniqueness of solution,
// and produces output of complete board that can be used with separate visualizer.
#include <iostream>
#include <vector>
using namespace std;
...
void solve(int r, int c) {
    ...
    if (debugRecursions % 500000000 == 0) {
...
int main(int argv, char** argc) {
    ...
    cin >> W >> H;
    if (W > MAX_WIDTH)
        cerr << "ILLEGAL WIDTH" << endl;
    if (H > MAX_HEIGHT)
        cerr << "ILLEGAL HEIGHT" << endl;
    ...
    for (int r=0; r < H+2; r++)
        for (int c=0; c < W+2; c++)
            if (alpha.find(original[r][c]) == string::npos)
                cerr << "INVALID CHARACTER IN INPUT" << endl;
    if (cin >> alpha)  // should intentionally fail on EOF
        cerr << "EXTRANEOUS INPUT AT END OF FILE" << endl;
