...
#ifdef TOLERANCE
    double tolerance;   // how close to zero was calculation that caused this event
#endif
    ...
    if (a > 0) {
...
#ifdef TOLERANCE
        result.tolerance = abs(det);  // determinant of zero means tangency
#endif
        ...
        if (det >= 0) {
            ...
            if (rootSmall > 0) {
                result.t = rootSmall;
#ifdef TOLERANCE
                result.tolerance = min(result.tolerance, abs(rootSmall));
#endif
            } else {
                result.t = rootBig;
...
#ifdef TOLERANCE
                result.tolerance = min(result.tolerance, abs(rootSmall));
                result.tolerance = min(result.tolerance, abs(rootBig));
#endif
