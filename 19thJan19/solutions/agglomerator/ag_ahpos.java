/*  ACM Mid-Central Programming competition 2015 Problem B: Agglomerator
    ...
    robustness check: test with pos, neg, 0 dEPS: here pos
*/

import java.util.*;
import static java.lang.Math.*;

 public class ag_ahpos {
...
class blobpos {  
   ...
   public blobpos(double x_, double y_, double vx_, double vy_, double r_, double t) {
      x = x_;
      y = y_; 
      vx = vx_; 
      vy = vy_;
   ...
   double dist(blobpos b) // when move older blob b to time ti
   {
      double dt = ti - b.ti;
      return hypot(b.x + dt*b.vx - x, b.y + dt*b.vy - y) - r - b.r;
   }
   // intersect newer blob
   double intersect (blobpos b) {  // change frame:  b fixed at origin, relative v
      double dt = b.ti - ti;
      move(dt);
   ...
   blobpos combine(blobpos b, double t) {  
      move(t - ti);
      ...
      return new blobpos(x*w + b.x*wb, y*w + b.y*wb, 
   ...
   public String toString() { // debug display only
    ...
    static PriorityQueue<glompos> pq = new PriorityQueue<glompos>(); 
    static ArrayList<blobpos> blobs = new ArrayList<blobpos>();
    static int n;
    static double lastT;
    ...
    static void solve(String[] args) {
      ...
      n = in.nextInt();
      for (int i = 0; i < n; i++)  
         addIntersections(new blobpos(in.nextDouble(), in.nextDouble(), 
    ...
    static void addIntersections(blobpos b) 
    {
         for (blobpos b2: blobs) 
            if (!b2.dead) {
              double t = b2.intersect(b); // intersect newer
              if (t < Double.POSITIVE_INFINITY) 
                  pq.offer(new glompos(t, b, b2));
            }
         blobs.add(b);
    }
}

class glompos implements Comparable<glompos>
{
    double t;
    blobpos b1, b2;

    public glompos(double t_, blobpos b1_, blobpos b2_)
    {                      // newer   older
      t = t_;
      b1 = b1_;
      b2 = b2_;
    ...
    public int compareTo(glompos g)
    {
       double dif = t - g.t;
       if (dif > 0) return 1;
       if (dif < 0) return -1;
